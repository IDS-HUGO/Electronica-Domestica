package com.electronica.tarjeta.repositories;
...
public class TarjetaRepository {
    ...
    public TarjetaRepository(DataSource dataSource) {
    ...
    public Tarjeta save(Tarjeta tarjeta) {
        ...
        try (Connection conn = dataSource.getConnection();
            ...
            throw new RuntimeException("Error al guardar tarjeta", e);
        }
    }
    ...
    public Optional<Tarjeta> findById(String id) {
        ...
        try (Connection conn = dataSource.getConnection();
            ...
            throw new RuntimeException("Error al buscar tarjeta", e);
        }
    }
    ...
    public List<Tarjeta> findAll() {
        ...
        try (Connection conn = dataSource.getConnection();
            ...
            throw new RuntimeException("Error al obtener tarjetas", e);
        }

        return tarjetas;
    }

    public Tarjeta update(String id, Tarjeta tarjeta) {
        String sql = """
            UPDATE registro_tarjetas 
            SET nombre_cliente = ?, numero_celular = ?, marca = ?, modelo = ?,
                problema_descrito = ?, tecnico_id = ?, tecnico_nombre = ?, estado = ?,
                fecha_finalizacion = ?, fecha_entrega = ?, updated_at = ?
            WHERE id = ?
        """;
        ...
        try (Connection conn = dataSource.getConnection();
            ...
            stmt.setString(6, tarjeta.getTecnicoId());
            stmt.setString(7, tarjeta.getTecnicoNombre());
            stmt.setString(8, tarjeta.getEstado());
            
            if (tarjeta.getFechaFinalizacion() != null) {
                stmt.setTimestamp(9, Timestamp.valueOf(tarjeta.getFechaFinalizacion()));
            } else {
                stmt.setNull(9, Types.TIMESTAMP);
            }
            
            if (tarjeta.getFechaEntrega() != null) {
                stmt.setTimestamp(10, Timestamp.valueOf(tarjeta.getFechaEntrega()));
            } else {
            ...
            tarjeta.setId(id);
            return findById(id).orElse(tarjeta);

        } catch (SQLException e) {
            throw new RuntimeException("Error al actualizar tarjeta", e);
        }
    }
    ...
    public void deleteById(String id) {
        ...
        try (Connection conn = dataSource.getConnection();
            ...
            throw new RuntimeException("Error al eliminar tarjeta", e);
        }
    }
